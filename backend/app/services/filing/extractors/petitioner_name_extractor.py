...
class PetitionerNameExtractor(BaseFieldExtractor):
    field_key = "petitioner_name"

    LABEL_PATTERNS = [
        re.compile(r"\bAPPLICANT\b\s*[-:~]?\s*(.+)", re.I),
        re.compile(r"\bPETITIONER\b\s*[-:~]?\s*(.+)", re.I),
        re.compile(r"\bAPPELLANT\b\s*[-:~]?\s*(.+)", re.I),
    ]
        ...
        for page in context.get("candidate_pages", []):
            ...
            for line in split_lines(text)[:60]:
                for pattern in self.LABEL_PATTERNS:
                    ...
                    raw = match.group(1)
                    cleaned = clean_party_name_candidate(raw)
                    if should_reject_name_candidate(cleaned):
                        continue
                    ...
                    candidates.append(
                        {
                            ...
                            "confidence": min(0.99, 0.74 * multiplier),
                            "source_type": "regex",
                            "page_from": page_no,
                            "page_to": page_no,
                            "chunk_id": None,
